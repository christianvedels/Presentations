...
.pull-left[
**Utopia: ML substitutes manual labour ...**

- Very low marginal cost; sometimes matches expert annotators (Korinek 2023; Gilardi et al. 2023)
- Linking, occupation coding, newspaper-scale extraction (Dahl et al. 2026; Dell et al. 2023)
- Text becomes a primary source: sentiment, beliefs, political stance at scale
]

.pull-right[
**Dystopia: predictions are not observations**

- ML errors are *systematic*, not classical measurement error
- Even above 90–95% accuracy can flip coefficients (Egami et al. 2024)
- Historical text is further from the training data: anachronism, archaic language, missing context
]

--

.pull-left-wide[
> *How should economic historians use ML without sacrificing credible inference?* **Our answer:** hand-code a **random** subsample, estimate the **structure of the bias**, and correct downstream estimates: a **debiasing framework** (Angelopoulos et al. 2023; Egami et al. 2024; Carlson & Dell 2025).
]

.pull-right-narrow[
.small123[**Validation alone cannot detect this.** Accuracy is silent on structural bias.]
]
...
.pull-left-wide[
**A common problem: many historical sources do not record gender**

- Research question: the gender wage gap among 3,271 Stockholm taxpayers in 1920 (Bengtsson & Molinder 2024)
- Can we construct gender from **first names** instead?
...
- A simple Naive Bayes classifier, trained on 2,000 Scandinavian name–gender pairs
- **81% accurate**
]
...
# Stockholm 1920: the results

.center[<img src="Figures/stockholm_coefplot.png" width="52%">]

--

.pull-left-wide[
- **Naive ML is confident but wrong:** $-0.71$ vs. the true $-0.82$. The errors are systematic: naming correlates with social status
- **Labeled-only** (random 10%, $n = 326$) is unbiased, but the CI is $3\times$ wider
]

.pull-right-narrow[
> **Here we know the truth:** the tax records report gender, so the oracle (dashed line) is observed.

.small123[*Bars: 95% CI. 97.6% precision, 67.2% recall: a third of women are classified as men.* [Setup in detail](#stockholm-setup)]
]
...
.pull-left-wide[
**Ingredients:** a corpus of $N$ observations, an ML predictor $f(X_i)$ for all of them, and a **random** subsample of size $n \ll N$ with gold-standard labels $Y_i$.

1. Label *all* $N$ observations with the ML model
2. Compare predictions to gold-standard labels on the $n$-sample $\Rightarrow$ *estimate the bias of $f$*
3. Estimate the quantity of interest from the predictions, corrected by the estimated bias

.small123[
$$\hat{\theta}_{\text{DB}} = \frac{1}{N-n}\sum_{i=n+1}^{N} f(X_i) - \underbrace{\frac{1}{n}\sum_{i=1}^{n}\big(f(X_i) - Y_i\big)}_{\text{bias correction}} \qquad \text{(PPI, mean of } Y\text{)}$$
]
]

.pull-right-narrow[
> **Unbiased by design, with no assumptions on the model.** The researcher controls the sampling probabilities, so the correction has expectation zero regardless of the model. Precision comes from all $N$ predictions.

.small123[
Estimators: **PPI** (Angelopoulos et al. 2023), **DSL** (Egami et al. 2023, 2024). Extends to regressions, fixed effects, IV, DiD, RD.

[Predicted regressor (DSL)](#dsl-regressor) · [PPI vs. DSL](#appendix-ppi-vs-dsl) · [Simulations](#appendix-simulation-lhs) · [Extensions](#beyond)
]
]
...
# Revisiting the example: debiasing works

.center[<img src="Figures/stockholm_dsl_coefplot.png" width="66%">]

.pull-left-wide[
- **DSL** with the same 10% gold-standard sample: $-0.81$, statistically indistinguishable from the oracle $(-0.82)$
- CI **36% narrower** than labeled-only: uses all 3,271 predictions while correcting their bias
- The gold standard is where **the careful historian** enters: source criticism before coding, documented codebook, expert labellers

[The numbers](#revisit-table) · [The careful historian](#careful-historian)
]

.pull-right-narrow[
> **ML and the historian are complements.** Debiasing is only as good as the historical craftsmanship in the gold-standard sample.
]

---
name: historical

# ML performs worse on historical data

.pull-left[
.center[<img src="Figures/accuracy_f1_over_time.png" width="80%">]
.small123[*A pre-trained BERT emotion classifier on 200 labelled texts, rewritten by an LLM into the English of each period. Accuracy falls steadily as the language becomes archaic.*]
]

.pull-right[
.center[<img src="Figures/llm_vs_human_accuracy_f1.png" width="80%">]
.small123[*GPT-5.5 vs. author hand-coding on 200 paragraphs from 17th-century books (EEBO). Accuracy looks high, but F1 is 0.69 overall and 0.60 for irony.*]
]

.pull-left-wide[
> Errors correlate with **period, language, region, and complexity** $\Rightarrow$ historical applications routinely fall below the accuracy at which sign flips are documented. [Why](#why-worse) · [Exercise 1](#exercise-bert) · [Exercise 2](#exercise-eebo)
]

---
name: what-to-do

# Three types of ML tasks in economic history

.pull-left-wide[
> **Type 1: ML to replace human annotators.** ML labels what humans *could* label, at scale: linkage, occupation coding, sentiment, topics. $\Rightarrow$ **Always debias.** Most applications in the field fall here. [Workflow](#type1-workflow)
]

--

.pull-left-wide[
> **Type 2: ML to account for missing data.** A source $\mathcal{S}$ where the variable is observed fills a target $\mathcal{T}$ where it is not (name-based proxies, occupational scores, imputed GDP). $\Rightarrow$ **If source–target relevance holds, validate within $\mathcal{S}$ and treat as Type 1; otherwise Type 3.** [Details](#type2-setup)
]

--

.pull-left-wide[
> **Type 3: ML for new measures.** Continuous proxies humans *cannot* produce (embedding-based innovation, sentiment, stance). No gold standard by construction. $\Rightarrow$ **Surrogate logic: convergent validity with several proxies + placebo independence.** [Embeddings](#type3-embeddings) · [Validation](#type3-validation)
]
...
# When can the bias be corrected? Putting it together

![Taxonomy flowchart](Figures/analysis_flowchart.png)

.small123[[Type 1](#type1-lit) · [Type 2](#type2-setup) · [Type 3](#type3-point)]
...
.panelset[
.panel[.panel-name[Debiasing workflow]
.pull-left-wide[
1. **Validate** (accuracy, precision, recall, F1) on a human-verified subset, but do not stop there
2. **Sample** $n$ observations with known, strictly positive $\pi_i$; uniform unless there is a reason to stratify; $n$ of 500 to 3,000 in practice
3. **Annotate** with historically trained experts; skill required rises with the ambiguity of the task
4. **Apply** the estimator: `dsl` (R), `ppi-python` (Python)
]
.pull-right-narrow[
> **Ambiguity scale:** numbers $<$ occupations $<$ semantics. The further right, the more the careful historian's craft matters.

.small123[[Full workflow](#type1-workflow)]
]
]
.panel[.panel-name[Scale]
.pull-left-wide[
.center[<img src="Figures/cost.png" width="100%">]
]
.pull-right-narrow[
.small123[
$$C_k(N) = F_k + c_k N$$
Fixed cost $F_k$ plus marginal cost $c_k$ per unit, for mode $k$
]

**How many hours by hand?**

- **Tens:** type it yourself
- **Hundreds to thousands:** off-the-shelf LLM / VLM
- **Thousands+:** build your own, and share it
- Validation and debiasing are never automated

.small123[[More](#scale)]
]
]
.panel[.panel-name[Digitization]
.pull-left-narrow[
.center[<img src="Figures/digitization.png" width="85%">]
]
.pull-right-wide[
- VLMs read the image directly and handle tables and layout
- Pass *both* the OCR/HTR text *and* the image; prompt the model never to invent text or numbers absent from the OCR
- On German patents 1877–1918, VLM error rates fall below those of research assistants (Grießhaber et al. 2025)
- Still validate against the original scans

.small123[[More](#digitization)]
]
]
.panel[.panel-name[Reproducibility]
.pull-left-wide[
- GPU floating-point arithmetic is non-associative; LLM output is not bit-identical across runs, even at temperature 0 with a seed
- Forcing determinism costs output quality and speed
- **Proposal: $\varepsilon$-reproducibility.** Like a careful RA labelling twice; editorial threshold of e.g. 1% on downstream coefficients
- The real threat is model deprecation: prefer locally stored open models

.small123[[More](#reproducibility)]
]
]
]
...
.footnote[.left[.small123[[Back to slides](#example-results)]]]
...
.footnote[.left[.small123[[Back to slides](#framework)]]]
...
.footnote[.left[.small123[[Back to slides](#solution)]]]
...
.footnote[.left[.small123[[Back to slides](#historical)]]]
...
.footnote[.left[.small123[[Back to slides](#what-to-do)]]]
...
.footnote[.left[.small123[[Back to slides](#best-practices)]]]
...
.footnote[.left[.small123[[Back to slides](#conclusion)]]]
