...
- This paper introduces a method for:  
  ...
  + **H**istorical **A**ccounts (e.g. wage tables)
  + **O**ccupational **S**cores (something like IPUMS occscore [but better])


### Some points:
- .chaosred[**Still preliminary**]
- Different than Saavedra & Twinam (2020) $\leftrightarrow$ Not a classical supervised ML approach
- Different than Paker, Stephenson, Wallis (2025) $\leftrightarrow$ Produces estimates across occupations
...
.panel[.panel-name[4 Broad]
.chaosred[**We generalise the process of doing economic history:**] 

- You have sources, and you want to summarise those sources to be informative for a target. 
- This depends on the relevance of each piece of information
]
...
.chaosred[**Demo: Get code running**]
...
$$
\mathbb{E}(y_i \mid h_j) = \sum_i y_i\Pr(y_i\mid h_j)
$$
]
]
...
2. By using Bayes we get that
$$
\mathbb{E}(y_i \mid h_j) = \sum_i y_i\frac{\Pr(h_j \mid y_i) \Pr(y_i)}{\Pr(h_j)}
$$
]
]
...
.pull-left[
.small123[
3. Now an entry in a wage table might look something like:
...
.pull-right[
.small123[
4. So we can naturally swap $y$ for $d$ getting

$$
\mathbb{E}(y_i \mid h_j) = \sum_i y_i\frac{\Pr(h_j \mid d_i) \Pr(d_i)}{\Pr(h_j)}
$$
*Note that $\Pr(h_j\mid d_i)$ is simply the probability of having a specific HISCO code given the occupation's description*
]
]
...
.pull-right[
.small123[
.chaosred[**Applying it to new sources**]  

5. Once we have $\mathbb{E}(y_i \mid h_j)$ we can estimate target outcomes via
$$
\mathbb{E}(y_k \mid d_k) = \sum_j \mathbb{E}(y_i \mid h_j) \Pr(h_j \mid d_k)
$$
]
]
...
# Mostly harmless maths (assumptions)
...
.small123[
#### Outputs a probability of belonging to standardized occupational codes
]
...
$\rightarrow$ Means that we can use PPI and DSL (Angelopoulos et al, 2023; Egami et al 2024)
]
...
# PPI style bias correction example (if time)

.pull-left-wide[
- Say we are interested in the mean of income: $\mu(y)$
- But we only observe $\hat{y}$
- And we have a small validation set, $k$, where we observe both $y$ and $\hat{y}$.
- $\hat{y}$ might be biased: $\mu(y)\not\approx \mu(\hat{y})$
- **Solution:**
...
.footnote[
.chaosred[*There are versions of this that generalisez into regression*]
]
...
# Bias correction via self-validation (if time)
...
#### Example
We estimate that:
> HISCO 83110 "Blacksmith" earns $2.52 a day:
...
.small123[
| $d_i$       | $Pr(83110\mid d_i)$ | $Pr(83120\mid d_i)$ | $Pr(87340\mid d_i)$ | $Pr(88050\mid d_i)$ | $Pr(80110\mid d_i)$ |
|-------------|----------------------|-----------------------|--------------------|---------------------|---------------------|
...
.small123[
| $d_i$       | $Pr(d_i\mid 83110)$ | $Pr(d_i \mid 83120)$ | $Pr(d_i \mid 87340)$ | $Pr(d_i \mid 88050)$ | $Pr(d_i \mid 80110)$ |
|-------------|------------|-------------|----------|-----------|-----------|
...
gets an estimated daily wage of $2.75.
